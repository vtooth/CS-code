...
public class MantoothV09
{
    public static void main(String[] args)
    {
        card[]deck = new card[52];
        card[]pulled = new card[0];
        deck[0] = new card("H", "A");
        deck[1] = new card("H", "2");
        deck[2] = new card("H", "3");
        deck[3] = new card("H", "4");
            ...
            if(choice == 1)
                {
                    ...
                    for(int s=0; s<pulled.length; s++)
                        pulled[s].display();
                } 
            if(choice == 2)
                {
                    for(int s = 0; s<deck2.length; s++)
                       deck2[s].display(); 
                    System.out.println("\n");
                }
            if(choice == 3)
                {   
                {deck2[0].pull();
                 points +=deck2[0].getpoints();
                 pulled = lengthen(pulled);
                 for(int s = 0; s<pulled.length; s++)
                 {
                    if(pulled[s] == null)
                     pulled[s] = deck2[0];
    ...
    public static card[] lengthen(card[]array)
    {
        card[]temp = new card[array.length+1];
        for(int s = 0; s<array.length; s++)
           temp[s] = array[s];
        return temp;
    }
    public static card[] shorten(card[]array)
    {
        card[]temp = new card[array.length-1];
